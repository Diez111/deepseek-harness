...
| `description` | string (optional) | One-line, active-voice summary of the command (5-10 words), for UI/log display only — no effect on execution. Omitted or empty: a deterministic label is derived from the command, so a missing display field never fails the call. |
